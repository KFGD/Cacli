...
##read from data base
        ...
        for i in sorted_dist:
                sorted_dataY.append(re_dataY[dist.index(i)])
                sorted_dataX.append(re_dataX[dist.index(i)])
                min_dist.append(i)
                if i>=max_dist:
                        break
 #       print(dist)
...
#	return query_string
	...
	WTM_Y=str(404040)
	WTM_X=str(196858.3485397672)
	WGS_Y=longtitude#str(37.559395)
	WGS_X=latitude#str(126.964443)
	#WTM is format of data's axis
	url_fromWTM="https://apis.daum.net/local/geo/transcoord?apikey=073ed517a575ae44700bfd48b65ce03b&fromCoord=WTM84&y="+WTM_Y+"&x="+WTM_X+"&toCoord=WGS&output=json"

	#WGS is format of user's axis
	url_fromWGS="https://apis.daum.net/local/geo/transcoord?apikey=073ed517a575ae44700bfd48b65ce03b&fromCoord=WGS84&y="+WGS_Y+"&x="+WGS_X+"&toCoord=WTM&output=json"
	...
	for i in range(0,len(WTM_Ys)):
		...
		url_fromWTM="https://apis.daum.net/local/geo/transcoord?apikey=073ed517a575ae44700bfd48b65ce03b&fromCoord=WTM84&y="+WTM_Ys[i]+"&x="+WTM_Xs[i]+"&toCoord=WGS&output=json"
